...
plot_dat <- picks %>% 
    mutate(exp_spent = ifelse(type == "call", 
           ...
           width = exp_made - exp_spent,
           wperc = width / exp_spent,
           stock_picked = 1:length(exp_made),
           cum_spent = cumsum(exp_spent),
           cum_made = cumsum(exp_made),
           diff = cum_made / cum_spent) %>% 
    select(stock_picked,
           cum_spent,
           cum_made, 
           diff) %>% 
    pivot_longer(-stock_picked, names_to = "type", values_to = "cumulative")

ggplot(plot_dat, aes(x = stock_picked, y = cumulative, color = type, group = type)) +
    geom_line() + 
    scale_y_continuous(trans = "log10") + 
    geom_text(aes(label = round(cumulative, 2)), 
              check_overlap = TRUE,
              vjust = 0)
...
```{r}
# picks <- read_csv("put_picks_04032023_mixed.csv")
...
for (pick in picks$symbol) {
    ...
    cor_ <- cor(corr_dat$spy_change, corr_dat$pick_change)
...
hist(cor_dat$correlation)
```
Long story short, there is correlation between all of these and I should 
adjust my expectations for it. Just not sure how to do that yet. 
