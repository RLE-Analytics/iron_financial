...
spy_hist <- spy_hist %>% 
    ...
    select(Date,
           spy_change)
cmcsa_hist <- py$get_hist_data("CMCSA", py$token)
cmcsa_hist <- cmcsa_hist %>% 
    mutate(last_close = lag(Close), 
           cmcsa_change = Close - last_close) %>% 
    select(Date,
           cmcsa_change)

corr_dat <- spy_hist %>% 
    inner_join(cmcsa_hist, by = "Date") %>% 
    filter(!is.na(spy_change) | !is.na(cmcsa_change))

cor(corr_dat$spy_change, corr_dat$cmcsa_change)
```
