...
mc_dat <- tibble(
    Date = seq.Date(start_date, Sys.Date() - days(1), by = "day"),
    perc_change = c(NA, sample(evd_dist$percent_change, 
                         size = 30, 
                         replace = TRUE) + 1),
    closing_price = c(start_price, rep(NA, 30))
)

for (da in 2:31) {
    mc_dat$closing_price[da] <- mc_dat$closing_price[da - 1] * 
        mc_dat$perc_change[da]
    
    mc_dat$closing_price[da] <- ifelse(mc_dat$closing_price[da] < 0, 0,
                                       mc_dat$closing_price[da])
}
...
mc_dat <- tibble(
    ...
    perc_change = sample(evd_dist$percent_change, 
                         size = 31 * num_sims, 
                         replace = TRUE) + 1,
    closing_price = rep(c(start_price, rep(NA, 30)), num_sims),
    sample = rep(1:num_sims, each = 31)
)

for (da in seq.Date(start_date, Sys.Date() - days(1), by = "day")) {
    da <- as_date(da)
    if (da == as_date("2022-10-14")) {
        next
    }
    
    mc_dat$closing_price[mc_dat$Date == da] <- mc_dat$closing_price[mc_dat$Date == da - days(1)] * 
        mc_dat$perc_change[mc_dat$Date == da]
}
...
dates <- sort(unique(mc_dat$Date))

for (num in 1:length(dates)) {
    da <- as_date(dates[num])
    yda <- as_date(dates[num - 1])
    
    if (da == as_date("2022-10-14")) {
        next
    }
    
    mc_dat$closing_price[mc_dat$Date == da] <- mc_dat$closing_price[mc_dat$Date == yda] * 
        mc_dat$perc_change[mc_dat$Date == da]
}
